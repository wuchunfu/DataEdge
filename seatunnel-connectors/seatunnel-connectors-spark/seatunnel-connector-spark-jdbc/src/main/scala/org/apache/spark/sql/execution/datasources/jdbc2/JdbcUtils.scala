...
object JdbcUtils extends Logging {
    ...
    mode match {
      case JDBCSaveMode.Update =>
        ...
        val duplicateSetting = rddSchema
          ...
          .map { name =>
            if (duplicateIncs.contains(name)) {
              s"$name=$name+VALUES($name)"
            } else {
              s"$name=VALUES($name)"
            }
          }.mkString(",")
  ...
  private def getCatalystType(
    ...
    val answer = sqlType match {
      ...
      case java.sql.Types.DECIMAL if precision != 0 || scale != 0 => DecimalType.bounded(precision, scale)
      ...
      case java.sql.Types.NUMERIC if precision != 0 || scale != 0 => DecimalType.bounded(precision, scale)
  ...
  def getSchema(
    ...
    while (i < ncols) {
      ...
      val columnType =
        dialect.getCatalystType(dataType, typeName, fieldSize, metadata)
  ...
  def resultSetToRows(resultSet: ResultSet, schema: StructType): Iterator[Row] = {
    val inputMetrics = Option(TaskContext.get()).map(_.taskMetrics().inputMetrics).getOrElse(new InputMetrics)
  ...
  private def makeGetter(dt: DataType, metadata: Metadata): JDBCValueGetter = dt match {
    ...
    case DecimalType.Fixed(p, s) =>
      (rs: ResultSet, row: InternalRow, pos: Int) =>
        val decimal = nullSafeConvert[java.math.BigDecimal](rs.getBigDecimal(pos + 1), d => Decimal(d, p, s))
