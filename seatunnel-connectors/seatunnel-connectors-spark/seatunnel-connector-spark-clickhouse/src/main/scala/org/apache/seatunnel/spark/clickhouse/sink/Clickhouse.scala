...
class Clickhouse extends SparkBatchSink {
  ...
  override def checkConfig(): CheckResult = {
    ...
    if (checkResult.isSuccess) {
      ...
      if (this.config.containsKey("fields")) {
        this.fields = config.getJSONArray("fields").asInstanceOf[util.List[String]]
  ...
  override def prepare(env: SparkEnvironment): Unit = {
    ...
    defaultConfig.put("retry_codes", new util.ArrayList())
...
object Clickhouse {
  ...
  class DistributedEngine(val clusterName: String, val database: String, val table: String) {
  }
