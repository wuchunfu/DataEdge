...
public final class EnvironmentUtil {
            ...
            if (config.containsKey(ConfigKeyName.RESTART_STRATEGY)) {
                ...
                switch (restartStrategy.toLowerCase()) {
                    case "no":
                        ...
                        break;
                    case "fixed-delay":
                        int attempts = config.getIntValue(ConfigKeyName.RESTART_ATTEMPTS);
                    ...
                    case "failure-rate":
                        ...
                        int rate = config.getIntValue(ConfigKeyName.RESTART_FAILURE_RATE);
    ...
    public static CheckResult checkRestartStrategy(JSONObject config) {
        if (config.containsKey(ConfigKeyName.RESTART_STRATEGY)) {
            ...
            switch (restartStrategy.toLowerCase()) {
                case "fixed-delay":
                    if (!(config.containsKey(ConfigKeyName.RESTART_ATTEMPTS) && config.containsKey(ConfigKeyName.RESTART_DELAY_BETWEEN_ATTEMPTS))) {
                    ...
                    break;
                case "failure-rate":
                    if (!(config.containsKey(ConfigKeyName.RESTART_FAILURE_INTERVAL) && config.containsKey(ConfigKeyName.RESTART_FAILURE_RATE) && config.containsKey(ConfigKeyName.RESTART_DELAY_INTERVAL))) {
